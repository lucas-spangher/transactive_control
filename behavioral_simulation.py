...
class Person:
    ...
    def __init__(self, workstation, name, df_data, starting_date, states=[], weights=[]):
        self.workstation = workstation
        self.name = name
        self.states = []
        self.weights = []
        self.starting_date = starting_date  ## TODO: Manan, is there a way to infer this from 
    ...
    def daily_weight_fit(self, date):
        ...
        objective = cvx.Minimize(
            cvx.sqrt(
                cvx.sum(
                        [cvx.square(flat_y[i] - C@z[i]) for i in range(len(flat_y))]
                    )
                )
            )
        constraints = []
...
to determine the impact of vicarious learning."""
    ...
    def vicarious_learning_average(self, timestamp):
        ...
        prev_hour_energy = [
            person.get_energy_at_time(timestamp, hour - 1) for person in self.people_list
        ]
    ...
    def daily_weight_fit(self, date):
        ...
        for person in self.people_list:
            print(person.name)
            person.state_weights, person.input_weights, person.states = person.daily_weight_fit(date)
            ...
            for person_name in filtered_df["Name"].unique():
                curr_person_list.append(
                    Person(
                        ...
                        name=person_name,
                        starting_date = pd.Timestamp("2018-09-06"), ## TODO: not hard coded 
                        df_data=[
                             df[
                                (df["Name"] == person_name)
