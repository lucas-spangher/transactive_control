...
class CheckpointCallback(BaseCallback):
    ...
    def __init__(
        self, save_freq: int, save_path: str, name_prefix="rl_model", verbose=0
    ):
    ...
    def _on_step(self) -> bool:
        if self.n_calls % self.save_freq == 0:
            path = os.path.join(
                self.save_path,
                "{}_{}_steps".format(self.name_prefix, self.num_timesteps),
            )
...
class StopTrainingOnRewardThreshold(BaseCallback):
    ...
    def _on_step(self) -> bool:
        assert self.parent is not None, (
            "`StopTrainingOnRewardThreshold` callback must be used "
        ...
        if self.verbose > 0 and not continue_training:
            print(
                "Stopping training because the mean reward {:.2f} "
                " is above the threshold {}".format(
