                    ...
                    if fprop_factor is not None:
                        if fprop_factor not in tmp_stats_cache:
                            if op_type == "Conv2D":
                                ...
                                if (
                                    ...
                                    slot_fprop_factor_stats2 = tf.Variable(
                                        tf.diag(tf.ones([fprop_factor2_size]))
                            ...
                            slot_fprop_factor_stats = tf.Variable(
                                tf.diag(tf.ones([fprop_factor_size]))
                                * self._diag_init_coeff,
                                name="KFAC_STATS/" + fprop_factor.op.name,
                                trainable=False,
                            )
                        ...
                        if not (
                            ...
                            if bprop_factor not in tmp_stats_cache:
                                slot_bprop_factor_stats = tf.Variable(
                                    tf.diag(tf.ones([bprop_factor.get_shape()[-1]]))
                                    * self._diag_init_coeff,
        ...
        for var in varlist:
                    ...
                    if (
                        ...
                        if op_type == "Conv2D" and not self._approx_t2:
                            ...
                            fprop_factor = tf.concat(
                                [
                                    fprop_factor,
                                    tf.ones([tf.shape(fprop_factor)[0], 1])
                                    / operator_height
                                    / operator_width,
                                ],
                                1,
                            ...
                            fprop_factor = tf.concat(
                                [fprop_factor, tf.ones([tf.shape(fprop_factor)[0], 1])],
                                1,
                            )
            ...
            for stats_var in bprop_stats_vars:
                if stats_var not in stats_updates_cache:
                    ...
                    ) / tf.cast(tf.shape(bprop_factor)[0], tf.float32)
    ...
    def apply_stats(self, stats_updates):
        ...
        if self._async_stats:
            ...
            update_stats_op = tf.cond(
                ...
                lambda: _update_running_avg_stats(stats_updates),
                _update_accum_stats,
            )
    ...
    def apply_stats_eigen(self, eigen_list):
        ...
        with tf.control_dependencies(update_ops):
            ...
            if KFAC_DEBUG:
                update_ops.append(
                    tf.Print(
                        tf.constant(0.0), [tf.convert_to_tensor("updated kfac factors")]
                    )
                )
        return update_ops
    ...
    def get_kfac_precond_updates(self, gradlist, varlist):
        ...
        for grad_1, var in zip(gradlist, varlist):
            ...
            if KFAC_DEBUG:
                ...
                tf.Print(
                    grad,
                    [tf.sqrt(tf.reduce_sum(tf.pow(grad, 2)))],
    ...
    def apply_gradients_kfac(self, grads):
        ...
        if self._async_eigen_decomp:
            ...
            enqueue_op = tf.cond(
                ...
                lambda: queue.enqueue(self.compute_stats_eigen()),
                tf.no_op,
            )
                ...
                if not self._async_eigen_decomp:
                    ...
                    update_factor_ops = tf.cond(
                        ...
                        lambda: tf.cond(
                            tf.equal(queue.size(), tf.convert_to_tensor(0)),
                            tf.no_op,
                            lambda: tf.group(*self.apply_stats_eigen(dequeue_op())),
                        ),
                        no_op_wrapper,
                    )
                    ...
                    def optim_op():
                        def update_optim_op():
                            if self._full_stats_init:
                                return tf.cond(
                                    tf.greater(
                                        self.factor_step, tf.convert_to_tensor(0)
                                    ),
                        ...
                        if self._full_stats_init:
                            ...
                            return tf.cond(
                                tf.greater_equal(self.sgd_step, self._cold_iter),
                                update_optim_op,
                                tf.no_op,
                            )
            ...
            if KFAC_DEBUG:
                with tf.control_dependencies([sgd_step_op, cold_optim_op]):
                    sgd_step_op = tf.Print(
                        sgd_step_op,
                        [self.sgd_step, tf.convert_to_tensor("doing cold sgd step")],
                    )
