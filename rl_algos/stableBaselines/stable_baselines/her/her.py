    ...
    def __init__(
        ...
        **kwargs
        ...
        if isinstance(goal_selection_strategy, str):
            assert (
                goal_selection_strategy in KEY_TO_GOAL_STRATEGY.keys()
            ), "Unknown goal selection strategy"
    ...
    def _check_obs(self, observation):
        if isinstance(observation, dict):
            if self.env is not None:
                ...
                raise ValueError(
                    "You must either pass an env to HER or wrap your env using HERGoalEnvWrapper"
                )
        return observation

    def predict(self, observation, state=None, mask=None, deterministic=True):
        return self.model.predict(
            self._check_obs(observation), state, mask, deterministic
        )

    def action_probability(
        self, observation, state=None, mask=None, actions=None, logp=False
    ):
        return self.model.action_probability(
