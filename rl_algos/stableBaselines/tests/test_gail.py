...
def test_generate_callable(tmp_path):
    ...
    generate_expert_traj(
        dummy_expert,
        tmp_path / "dummy_expert_cartpole",
        env,
        n_timesteps=0,
        n_episodes=10,
    )
...
def test_gail_callback(tmp_path):
    dataset = ExpertDataset(
        expert_path=EXPERT_PATH_PENDULUM,
        traj_limitation=10,
        sequential_preprocessing=True,
        verbose=0,
    )
    ...
    checkpoint_callback = CheckpointCallback(
        save_freq=500, save_path=str(tmp_path / "logs/gail/"), name_prefix="gail"
    )
...
@pytest.mark.parametrize(
    "model_class", [A2C, ACKTR, GAIL, DDPG, PPO1, PPO2, SAC, TD3, TRPO]
)
...
def test_behavior_cloning_discrete(tmp_path, model_class):
    dataset = ExpertDataset(
        expert_path=EXPERT_PATH_DISCRETE,
        traj_limitation=10,
        sequential_preprocessing=True,
        verbose=0,
    )
