...
def test_load_parameters(request, model_class):
    ...
    assert not np.any(np.isclose(original_actions_probas, new_actions_probas)), (
        "Action probabilities did not change " "after changing model parameters."
    )
    ...
    assert not np.any(np.isclose(new_actions_probas, new_actions_probas_list)), (
        "Action probabilities did not "
    ...
    try:
        ...
        new_actions_probas = model.action_probability(observations, actions=actions)
        assert np.all(np.isclose(original_actions_probas, new_actions_probas)), (
            "Action probabilities changed " "after load_parameters from a file."
        )
        ...
        new_actions_probas = model.action_probability(observations, actions=actions)
        assert np.all(np.isclose(original_actions_probas, new_actions_probas)), (
            "Action probabilities changed after" "load_parameters from a file-like."
        )
    finally:
