...
def find_tf_dependency():
    ...
    if install_tf:
        tf_dependency = (
            ["tensorflow-gpu>=1.8.0,<2.0.0"] if tf_gpu else ["tensorflow>=1.8.0,<2.0.0"]
        )
        if tf_gpu:
...
setup(
    ...
    packages=[
        package for package in find_packages() if package.startswith("stable_baselines")
    ],
    package_data={"stable_baselines": ["py.typed", "version.txt"],},
    install_requires=[
    ...
    extras_require={
        "mpi": ["mpi4py",],
        "tests": ["pytest", "pytest-cov", "pytest-env", "pytest-xdist", "pytype",],
