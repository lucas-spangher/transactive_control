...
def get_first_param(pop_size, param_num, encode_length):
    ...
    print "Initialization parameter population"
...
def get_rmse(param,reframed):
    ...
    # training
    history = model.fit(train_X, train_y, epochs = 25, batch_size = 4096, validation_data = (valid_X, valid_y), verbose = 1, shuffle = True)
    ...
    inv_yhat = concatenate((yhat, valid_X[:, -11:]), axis=1)
    inv_yhat = scaler.inverse_transform(inv_yhat)
    inv_yhat = inv_yhat[:,0]

    # inverse transform of real value
    valid_y = valid_y.reshape((len(valid_y), 1))
    inv_y = concatenate((valid_y, valid_X[:, -11:]), axis=1)
...
reframed.drop(reframed.columns[[index_col+1, index_col+2, index_col+3, 
    index_col+4, index_col+5,
    index_col+6,index_col+7]], axis = 1, inplace = True)
